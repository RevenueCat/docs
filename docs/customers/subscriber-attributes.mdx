...
:::warning
Since subscriber attributes are writable using a [public key](/docs/authentication) they should not be used for managing secure or sensitive information such as subscription status, coins, etc.
:::
...
| `$firebaseAppInstanceId` | [Firebase](/docs/firebase-integration) instance identifier                                      |
...
You can access subscriber attributes through the [REST API](https://docs.revenuecat.com/reference) using a secret key, in [webhooks](/docs/webhooks), and through analytics integrations ([Amplitude](/docs/amplitude), [Mixpanel](/docs/mixpanel), [Segment](/docs/segment)). The [customer view dashboard](/docs/customers) will also show a list of attributes for the individual user that you can edit.
...
Subscriber attributes are also included with transaction data for [Scheduled Data Exports](/docs/scheduled-data-exports).
:::

## Next Steps

- Enrich your app by [reacting to the user's current subscription status ](/docs/purchaserinfo)