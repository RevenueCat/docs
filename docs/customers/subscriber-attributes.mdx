...
:::warning
Since subscriber attributes are writable using a [public key](/authentication) they should not be used for managing secure or sensitive information such as subscription status, coins, etc.
:::
...
| `$firebaseAppInstanceId` | [Firebase](/firebase-integration) instance identifier                                      |
...
Any attribute can be cleared by passing `null` or an empty string as the key value. Individual attributes can also be cleared for a specific user in their [customer view](/basic-information).
...
You can access subscriber attributes through the [REST API](https://docs.revenuecat.com/reference) using a secret key, in [webhooks](/webhooks), and through analytics integrations ([Amplitude](/amplitude), [Mixpanel](/mixpanel), [Segment](/segment)). The [customer view dashboard](/basic-information) will also show a list of attributes for the individual user that you can edit.
...
Subscriber attributes are also included with transaction data for [Scheduled Data Exports](/scheduled-data-exports).
:::

## Next Steps

- Enrich your app by [reacting to the user's current subscription status ](/customer-info)