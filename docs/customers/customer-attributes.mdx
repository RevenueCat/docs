...
By default, attributes are only synced with RevenueCat servers when Purchases.configure() is called, app backgrounded, and when purchases are made or restored; though the `SyncPurchasesAndOfferingsIfNeeded()` method can also be used if custom attributes are being used for Targeting. [Learn more here.](/tools/targeting/custom-attributes)
:::
