...
By default, Attributes are only synced with RevenueCat servers when Purchases.configure() is called, app backgrounded, and when purchases are made or restored.

When custom attributes are being used for Targeting, they'll be synced more frequently on supported SDK versions. [Learn more](<-- some hyperlink -->)
:::

## How Attributes can be used

1. Attributes can be fetched by API for individual customers, or reviewed in their Customer Profile in the Dashboard.
...
You can specify up to 50 unique custom attributes, with key names up to 40 characters long and values up to 500 characters long. Keys cannot start with `$`, since that prefix is withheld for reserved attributes (see below).
...
Any attribute can be cleared by passing `null` or an empty string as the key value. Individual attributes can also be cleared for a specific user in their [customer view](/dashboard-and-metrics/customers-group/basic-information).
