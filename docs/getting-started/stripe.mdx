...
Before launching your Stripe integration, be sure to read the limitations that apply to [working with web payments](/docs/stripe#section-working-with-web-payments).
...
Add your Stripe products to your project's [entitlements](/docs/entitlements) to easily group the products for your app. RevenueCat will send Stripe subscriptions to your app the same way we do for the other app stores.

![](https://files.readme.io/fd4e1ab-Screen_Shot_2021-12-01_at_11.23.21_AM.png "Screen Shot 2021-12-01 at 11.23.21 AM.png")

:::info
In order for Stripe subscriptions to unlock [entitlements](/docs/entitlements), you must set a Product Identifier in the desired Offering to match a Stripe **product ID** exactly.
:::
...
Pricing in RevenueCat [Charts](/docs/charts) and [customer events](/docs/customer-history) will reflect any coupons applied to a Stripe purchase.
:::
...
When a customer encounters an issue with their payment, RevenueCat will generate a billing issue event in the RevenueCat [Customer History](/docs/customer-history). If all retries for a payment fails, you have three options:
