...
We recommend using a company account when registering for RevenueCat and setting up your app within a project. You'll be able to invite the rest of your team as [collaborators](/docs/collaborators) to your project, but **only the project owner can manage billing**. Project collaborators can't manage billing details.
...
:::info
If you try making purchases without a bundle ID and valid credentials, the SDK will fail with a '[receipt is not valid](/docs/errors#---invalid_receipt)' error as RevenueCat will be unable to validate the transaction with the store.
:::
...
:::info Here's a tip!
After registering your app, we recommend setting up [Platform Server Notifications](/docs/server-notifications). These notifications aren't required but will speed up [webhooks](/docs/webhooks) and integration delivery times and reduce lag time updating your subscribers.
:::
...
Service credentials need to be set up for RevenueCat to communicate with the app stores on your behalf. See our guides [App Store Connect Shared Secret](/docs/itunesconnect-app-specific-shared-secret), [App Store In-App Purchase Key](/docs/in-app-purchase-key-configuration), [Play Service Credentials](/docs/creating-play-service-credentials), and [Amazon Appstore Shared Secret](/docs/amazon-appstore) for more information.
...
Before you can start using RevenueCat to fetch products, you must configure your products in the respective stores. See the following guides for [App Store Connect](/docs/ios-products), [Google Play Console](/docs/android-products), [Amazon Appstore](/docs/amazon-product-setup), and [Stripe](/docs/stripe-products) for help navigating through this process.
...
Read more about setting up StoreKit Configuration files in our [Sandbox Testing](/docs/apple-app-store#ios-14-only-testing-on-the-simulator) guide.
:::

### ▶️ Configure Products and Entitlements in RevenueCat

Once your in-app products have been configured in [App Store Connect](/docs/ios-products), [Google Play Console](/docs/android-products), [Amazon Appstore](/docs/amazon-product-setup), or [Stripe](/docs/stripe-products), you'll need to copy that configuration into the RevenueCat dashboard. RevenueCat uses an Entitlements system to control access to premium features, and Offerings to manage the set of products you offer to customers.
...
See [Configuring Products](/docs/entitlements) to set up your products and then organize them into Offerings or Entitlements.
...
- [iOS/watchOS/tvOS Installation](/docs/ios)
- [Android Installation ](/docs/android)
- [React Native Installation ](/docs/reactnative)
- [Flutter Installation ](/docs/flutter)
- [Capacitor / Ionic Installation ](/docs/ionic)
- [Cordova Installation ](/docs/cordova)
- [Unity Installation ](/docs/unity)
- [macOS / Catalyst Installation ](/docs/macos)

If you run into issues with the SDK, see [Troubleshooting the SDKs](/docs/troubleshooting-the-sdks) for guidance.
...
See our guide on [Configuring SDK](/docs/configuring-sdk) for more information and best practices.

Make sure you configure _Purchases_ with your public SDK key only. This API key can be found in the API Keys Project settings page. You can read more about the different API keys available in our [Authentication guide](/docs/authentication).
...
The `app_user_id` field in `.configure` is how RevenueCat identifies users of your app. You can provide a custom value here or omit it for us to generate an anonymous id. For more information, see our [Identifying Users](/docs/user-ids) guide.

When in development, we recommend enabling more verbose debug logs. For more information about these logs, see our [Debugging](/docs/debugging) guide.

If you're planning to use RevenueCat alongside your existing purchase code, check out our guide on [Observer Mode](/docs/observer-mode).
...
The SDK will automatically fetch the [configured Offerings](/docs/entitlements#offerings) and retrieve the product information from Apple, Google, or Amazon. Thus, available products will already be loaded when customers launch your purchase screen.

Below is an example of fetching Offerings. You can utilize Offerings to organize your paywall screen. See our guide on [Displaying Products](/docs/displaying-products) for more information and best practices.
...
If fetching your [Offerings](/docs/entitlements#offerings), [products](/docs/entitlements#products), or available [packages](/docs/entitlements#adding-packages) are empty, it's due to some configuration issue in the respective store.
...
The code sample below shows the process of purchasing a package and confirming it unlocks the "your_entitlement_id" content. More detail about the `purchase:package` method can be found in our guide on [Making Purchases](/docs/making-purchases).
...
:::info Here's a tip!
You can access a lot more information about a subscription than simply whether it's active or not. See our guide on [Subscription Status](/docs/customer-info) to learn if subscription is set to renew, if there's an issue detected with the user's credit card, and more.
:::
...
If two different [App User IDs](/docs/user-ids) restore transactions from the same underlying store account (Apple, Google, or Amazon account) RevenueCat may attempt to create an alias between the two App User IDs and count them as the same user going forward. See our guide on [Restoring Purchases](/docs/restoring-purchases) for more information on the different configurable restore behaviors.
...
**[View Samples ](/docs/sample-apps)**

## Next Steps

- If you haven't already, make sure your products are configured correctly by checking out our [guide on entitlements ](/docs/entitlements).
- If you want to use your own user identifiers, read about [setting app user IDs ](/docs/user-ids).
- If you're moving to RevenueCat from another system, see our guide on [migrating your existing subscriptions ](/docs/migrating-existing-subscriptions).
- Once you're ready to test your integration, you can follow our guides on [testing and debugging ](/docs/debugging).
- If you qualify for the App Store Small Business Program, check out our guide on [how to apply and inform RevenueCat ](/docs/app-store-small-business-program)
