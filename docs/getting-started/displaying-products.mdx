...
import rcBillingDisplayingProducts from "!!raw-loader!@site/code_blocks/getting-started/rc-billing/current-offering.ts";
...
<RCCodeBlock tabs={[
    ...
    { type: 'ts', content: rcBillingDisplayingProducts, name: 'Web (JS/TS)' },
]} />
...
<RCCodeBlock tabs={[
    ...
    { type: 'ts', content: rcBillingCustomOffering, name: 'Web (JS/TS)' },
]} />
...
Each Package includes an underlying product that includes more information about the price, duration, and other metadata. You can access the product via the `storeProduct` property (or `rcBillingProduct` property for [RevenueCat Billing](/getting-started/revenuecat-billing)):
