...
hidden: false
---
If you've [configured Offerings](/docs/entitlements) in RevenueCat, you can control which products are shown to users without requiring an app update. Building paywalls that are dynamic and can react to different product configurations gives you maximum flexibility to make remote updates.

![](https://files.readme.io/edbe93c-Screen_Shot_2019-09-20_at_8.42.23_AM.png "Screen Shot 2019-09-20 at 8.42.23 AM.png")

:::info
Before products and offerings can be fetched from RevenueCat, be sure to initialize the Purchases SDK by following our [Quickstart](/docs/getting-started) guide.
:::

## Fetching Offerings

Offerings are fetched through the SDK based on their [configuration](/docs/entitlements) in the RevenueCat dashboard.
...
For example, displaying a higher priced Offering to users that came from [paid acquisition](/docs/attribution) to help recover ad costs, or a specific Offering designed to show [iOS Subscription Offers](/docs/ios-subscription-offers) when a user has [cancelled their subscription](/docs/purchaserinfo#section-get-entitlement-information). 
...
- Now that you've shown the correct products to users, time to [make a purchase ](/docs/making-purchases)
- Check out our [sample apps ](/docs/sample-apps) for examples of how to display products.
