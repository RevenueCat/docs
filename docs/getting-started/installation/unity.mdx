...
If you prefer, you can also use OpenUPM-CLI to add the scoped registry and the package through the command line. To do that, install the [OpenUPM-CLI](https://openupm.com/docs/getting-started/getting-started.html) if you haven't already, then run `openupm add com.revenuecat.purchases-unity`. That should be it!
:::
...
<RCCodeBlock tabs={[
    { type: 'unity', content: unityCSharpContent, name: 'Unity' }
]} />
...
<RCCodeBlock tabs={[
    { type: 'groovy', content: unityGroovyContent, name: 'Groovy' }
]} />
...
* Now that you've installed the Purchases SDK in your Unity app, get started [building with Purchases :fa-arrow-right:](/docs/getting-started/getting-started#4-using-revenuecats-purchases-sdk)
