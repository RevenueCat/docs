...
On Android, RevenueCat **will not** consume or acknowledge any purchase if [finishing transactions](/migrating-to-revenuecat/finishing-transactions) is disabled, so be sure your app is handling that. **Failure to do so will result in your purchases being refunded after 3 days.**
:::
