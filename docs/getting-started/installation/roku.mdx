...
## Introduction
RevenueCat's Roku support allows you to easily offer subscriptions on Roku's streaming platform. This feature is currently in beta and has a number of known limitations.
...
This also means for any events RevenueCat dispatches containing an [`environment` field](/docs/integrations/webhooks/event-types-and-fields#events-format), the Roku channel where the purchase was made will determine whether the value is set to `SANDBOX` or `PRODUCTION`.
