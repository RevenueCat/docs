...
<RCCodeBlock tabs={[
    { type: 'kotlin', content: versionsContent, name: 'libs.versions.toml' }
]} />
...
In this scenario, we can specify the transitive dependency on `PurchasesHybridCommon` using Gradle. To do so, add the following to your `libs.versions.toml`: 

import commonVersion from "!!raw-loader!@site/code_blocks/getting-started/installation/kmp_4.toml";

<RCCodeBlock tabs={[
    { type: 'groovy', content: commonVersion, name: 'libs.versions.toml' }
]} />
...
In this scenario, we need to manually link the transitive dependency on `PurchasesHybridCommon`. One way to do so is using CocoaPods. This is useful if you're already using CocoaPods in your iOS app for dependencies other than your own Kotlin Multipatform module. 
