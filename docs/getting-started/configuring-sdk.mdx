...
hidden: false
---
If this is your first time integrating RevenueCat into your app, we recommend following our [Quickstart](/quickstart) guide.

:::info Using an older SDK (v3.x)
View our migration guide to v4.x [here](/ios-native-3x-to-4x-migration).
:::
...
Once you've [installed](/installation) the SDK for your app, it's time to initialize and configure it.
...
Make sure you configure _Purchases_ with your public SDK key only. You can read more about the different API keys available in our [Authentication guide](/authentication).
...
In certain cases, calling `configure` in `application:didFinishLaunchingWithOptions:` should be avoided. See [Identifying Users](/user-ids#provide-app-user-id-on-configuration) for more information.
:::

:::info SwiftUI App Protocol
Using SwiftUI's App protocol? Check out our [resource](/swiftui-helpers) on setting up the _Purchases_ SDK with SwiftUI.
:::
...
- **App User ID (optional)**: An identifier for the current user. Pass `null` if you don't have a user identifier at the time of configuration and RevenueCat will generate an anonymous Id for you. See our [guide on identifying users](/user-ids) for more information.
- **Observer Mode (optional)**: A boolean value to set _Purchases_ in [Observer Mode](/observer-mode). Only pass true if you're using RevenueCat for analytics purposes, and you're handling making purchases and checking subscription status yourself.
