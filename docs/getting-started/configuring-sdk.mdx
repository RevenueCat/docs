...
We have had reports of our API getting blocked in mainland China. If your app has a significant user base in mainland China, we recommend setting the `proxyURL` property to `https://api.rc-backup.com/` before initializing the RevenueCat SDK. Make sure this happens before configuring the SDK. 
