...
The process to start a subscription is initiated by calling the [purchase method in the SDK](/web/revenuecat-billing#purchasing-a-package). The subscription starts only if and when the initial payment is confirmed – if the customer cancels the checkout form, or if the initial payment for the subscription fails, no subscription is generated, and no events will be generated.
