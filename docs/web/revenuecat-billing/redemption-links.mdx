...
Handling purchases with Redemption Links means that you do not need to provide an App User ID when initializing the purchase, and customers can therefore check out anonymously. The purchase is then assigned to the logged in customer when they're directed into the mobile app _after_ the purchase.
:::
...
1. On tapping the redemption link, customers are linked into the mobile app (using a custom URL scheme).
...
| Identified         | Identified         | Follows project's [restore behavior](/getting-started/restoring-purchases#transferring-purchases-seen-on-multiple-app-user-ids) (default: transfer the purchase to the new App User ID) |

In most cases, the App User IDs will be merged (aliased) and treated as the same customer going forward. The exception is when both App User IDs are identified - in this case, it follows your project's configured [restore behavior](/getting-started/restoring-purchases#transferring-purchases-seen-on-multiple-app-user-ids).
...
| :----------------------- | :------------------------------- |
| purchases-android        | 8.10.5 and above                 |
| purchases-ios            | 5.13.0 and above                 |
...
You can get this in the RevenueCat dashboard, inside your RevenueCat Billing app’s settings, under “Redemption Links”

![](/images/rc-billing/redemption-links-dashboard-disabled.png)
...
![](/images/rc-billing/redemption-links-xcode-1.png)
...
import redemptionAndroidManifest from "!!raw-loader!@site/code_blocks/web/revenuecat-billing/redemption_androidmanifest.xml";

<RCCodeBlock tabs={[
    { type: 'xml', content: redemptionAndroidManifest, name: 'AndroidManifest.xml' }
]} />

You can check out official docs by Google [here](https://developer.android.com/training/app-links/deep-linking#adding-filters).
...
import swiftUIModifier from "!!raw-loader!@site/code_blocks/web/revenuecat-billing/redemption_swift_ui_modifier.swift";
...
import alternativeSwift from "!!raw-loader!@site/code_blocks/web/revenuecat-billing/redemption_alternative_swift.swift";
...
import androidRedemption1 from "!!raw-loader!@site/code_blocks/web/revenuecat-billing/redemption_android.kt";
...
![](/images/rc-billing/redemption-links-dashboard-sandbox.png)

4. Use the sandbox purchase flow to make a test purchase on a mobile device where your app is installed (either through [Web Paywall Links](/web/revenuecat-billing/web-paywall-links) sandbox URL, or the [Web SDK](/web/revenuecat-billing) with a sandbox API key).