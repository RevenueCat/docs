...
:::warning Apple Pay & Google Pay payment methods not currently supported
Due to issues with the way that wallet payments are presented to customers in the RevenueCat Billing checkout, we do not recommend enabling either Apple Pay or Google Pay. If you're actively using them on production, we recommend removing them as an option by deleting the approved domains (below) in your Stripe account.
:::
