...
- There is no support for product changes (upgrades, downgrades, or plan switches) mid-cycle. Customers purchasing a different subscription will create a separate subscription instead of replacing the existing one.
