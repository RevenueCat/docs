...
slug: overview
excerpt: Overview of RevenueCat's integration paths for web purchases
hidden: false
---
...
Web Billing currently has a the following limitations:

- There is no support for coupon codes for discounts ([introductory offers](product-setup#introductory-period) are supported).
- Support for product downgrades (switching to lower-priced products) is coming soon. Currently, customers need to cancel their existing subscription and re-subscribe to the new product.
