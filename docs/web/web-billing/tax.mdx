...
If you enable Stripe Tax in your RevenueCat integration, Stripe will charge you a [per-transaction fee](https://stripe.com/tax/pricing), for transactions where taxes are collected. This is in addition to the base Stripe transaction fee.
...
As soon as you've added a registration in the Stripe dashboard, Web Billing will start applying taxes for customers in that location.

:::warning Adding tax registrations impacts existing subscriptions
When you add a tax rate for a given location, any relevant taxes will apply for all future transactions in that location, including renewals of existing subscriptions. In the case of US & Canada where tax exclusive pricing is used, this will result in an increase in price for those customers. We recommend communicating this change in advance to any affected customers.
:::
...
As a test case, with the product tax category set to **General - Electronically Supplied Services** (`txcd_10000000`), you can check out with a **Texas** billing address, you should see a tax rate applied (6.25%).
