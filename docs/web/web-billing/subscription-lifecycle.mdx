...
excerpt: Web Billing subscription lifecycle
hidden: false
---
...
![](/images/web-billing/subscription-lifecycle.png)

## Starting a subscription

The process to start a subscription is initiated by calling the [purchase method in the SDK](/web/web-billing#purchasing-a-package). The subscription starts only if and when the initial payment is confirmed – if the customer cancels the checkout form, or if the initial payment for the subscription fails, no subscription is generated, and no events will be generated.
...
Web Billing does not yet support any offers or discounts.
...
The following trial eligibility can be set up per product. If a customer is eligible for the trial, it will be applied automatically.
...
![](/images/web-billing/billing-retry-and-grace-period.png)
...
Uncancelations occur when a customer reactivates auto-renewal (via the [customer portal](./customer-portal)) for a subscription that was previously canceled but that didn't expired yet. When this happens, RevenueCat generates an `UNCANCELLATION` event. The subscription continues without interruption and the customer maintains their entitlements throughout the process.
