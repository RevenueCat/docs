...
The metadata will be propagated to webhook events and the Stripe Customer object to track additional information about the purchase.

Thanks to the new metadata functionality, `purchases-js` will also automatically collect any `utm` parameter in the URL and send it as metadata when invoking `purchase()`.
...
You can opt-out from the automatic collection of UTM parameters by setting the `autoCollectUTMAsMetadata` flag to `false` in the `configure` method.
Here's an example:
