...
4. Use the sandbox purchase flow to make a test purchase on a mobile device where your app is installed (either through [Web Paywall Links](/web/web-billing/web-paywall-links) sandbox URL, or the [Web SDK](/web/web-billing) with a sandbox API key).
