...
excerpt: Automatically extend subscriptions as a promotional offer when customers make a purchase
hidden: false
---

Promotional subscription extensions allow you to reward customers with additional subscription time when they purchase through a specific offering or promotional campaign. This is a powerful tool for running time-limited promotions, partnership deals, or customer retention campaigns.
...
| Considerations        | Details                                                   |
| :-------------------- | :-------------------------------------------------------- |
| Extension method      | Set a new expiry timestamp (must be in the future)        |
| Maximum extension     | Up to one year per request                                |
...
import webhookSetup from "@site/code_blocks/guides/promotional-extension-webhook-setup.ts.txt?raw";

<RCCodeBlock
  tabs={[{ type: "ts", content: webhookSetup, name: "Webhook Endpoint" }]}
/>
...
import webhookPayload from "@site/code_blocks/guides/promotional-extension-webhook-payload.json?raw";

<RCCodeBlock
  tabs={[
    { type: "json", content: webhookPayload, name: "INITIAL_PURCHASE Webhook" },
  ]}
/>
...
import platformSwitch from "@site/code_blocks/guides/promotional-extension-platform-switch.ts.txt?raw";

<RCCodeBlock
  tabs={[{ type: "ts", content: platformSwitch, name: "Platform Router" }]}
/>
...
import appleRequest from "@site/code_blocks/guides/promotional-extension-apple-request.ts.txt?raw";

<RCCodeBlock
  tabs={[{ type: "ts", content: appleRequest, name: "Apple Extension" }]}
/>
...
import googleRequest from "@site/code_blocks/guides/promotional-extension-google-request.ts.txt?raw";

<RCCodeBlock
  tabs={[{ type: "ts", content: googleRequest, name: "Google Deferral" }]}
/>
...
| Parameter        | Type    | Description                                          |
| :--------------- | :------ | :--------------------------------------------------- |
| `expiry_time_ms` | integer | New expiration timestamp in milliseconds since epoch |

:::tip Calculating the New Expiration
For Google, you must calculate the new expiration time yourself. Take the current `expiration_at_ms` from the webhook and add your extension period in milliseconds.
:::
...
import fullHandler from "@site/code_blocks/guides/promotional-extension-handler.ts.txt?raw";

<RCCodeBlock
  tabs={[{ type: "ts", content: fullHandler, name: "Complete Handler" }]}
/>
