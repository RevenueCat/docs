...
If you would like to keep things short and simple as well as use DEV, TEST, and PROD environments, you can do this by simply setting up one [project](/docs/projects/overview) in RevenueCat. RevenueCat will automatically determine if a purchase is from the [Sandbox](https://www.revenuecat.com/docs/test-and-launch/sandbox) environment or Production. 
...
For teams looking to maintain complete separation of data across all environments, you can create separate [RevenueCat projects](/docs/projects/overview) for each environment that you plan to support. DEV, TEST, and PROD environments are the most common that we see. 
