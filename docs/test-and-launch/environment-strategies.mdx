...
## Single Project for Each Environment ##
...
For teams looking to maintain complete separation of data across all environments, you can create separate [RevenueCat projects](https://www.revenuecat.com/docs/welcome/projects#configuring-projects) for each environment that you plan to support. DEV, TEST, and PROD environments are the most common that we see. We recommend this approach to ensure that each environment has different API keys assosiated with it and they are seperated from one another. Once you setup a seperate project for each environment, you will be able to deceipher which events are coming from certain environments as well as keep all transactions from different environments completely seperated. Key details to this approach include:
