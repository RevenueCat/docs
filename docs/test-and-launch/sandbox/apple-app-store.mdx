...
![Image](https://files.readme.io/7a32b3e-984a1f91-3d0c-4f36-b020-b48275dfbcf4.png)

## Testing on Device

This section is for testing on a physical device. If you want to test on the simulator, skip to the next section for instructions on how to get started with [StoreKit testing](/docs/apple-app-store#ios-14-only-testing-on-the-simulator).
...
![Add your sandbox account under iOS settings to streamline testing](https://files.readme.io/527a407-IMG_3EC533BFA443-1.jpeg)
...
![Search for "storekit" to find the configuration file template quicker.](https://files.readme.io/37ff24b-Screen_Shot_2020-12-01_at_11.56.19_AM.png)

![Add the configuration file to your targets and save.](https://files.readme.io/abdf69a-Screen_Shot_2020-12-01_at_11.58.59_AM.png)
...
In the scheme editor, add the StoreKit Configuration file you created in [Step 1](/docs/apple-app-store#step-1-add-a-storekit-configuration-file) and click **Close**.
...
Even though products don't have to be set up in App Store Connect, they have to be set up in RevenueCat so that RevenueCat can validate the sandbox receipt. Refer to the [Configuring Products](/docs/entitlements) guide for more information.
:::
...
For sandbox and production [subscription offers](/docs/ios-subscription-offers), they are signed using the [In-App Key](/docs/in-app-purchase-key-configuration). However, when using a StoreKit Configuration file, the key used to sign offers must be exported from Xcode directly.
...
When testing, it may be helpful to delete a customer and all their receipts from RevenueCat to simulate a new installation. You can delete a specific user from the customer dashboard in RevenueCat. See our [docs on deleting users](/docs/manage-users) for more information.
