...
**Resolution:** 
If this error occurs before a purchase, ensure you're using the [correct API Key](/welcome/authentication) from the RevenueCat dashboard.
...
**Resolution:**
Make sure that there are no formatting issues when [setting subscriber attributes](/customers/subscriber-attributes#restrictions).
_______________________________________________________________________________
#### 😿 `NETWORK_ERROR`
**Problem:** 
...
**Problem:**
Observer Mode is [not currently supported](/migrating-to-revenuecat/observer-mode) for Amazon.
...
* You're set up to handle errors, time to start [making purchases in sandbox ➡️](/test-and-launch/sandbox)