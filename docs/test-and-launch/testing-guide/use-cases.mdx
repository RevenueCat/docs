...
- [Making purchases](docs.revenuecat.com/getting-started/making-purchases)
...
5. Open up the Revenuecat [overview](app.revenuecat.com/overview) page and check to see if you see a recent purchase for this product with the app user ID of the user that you configured the SDK with.

**Expected Results:** Purchase goes through successfully, user has the entitlement, and you are able to see the purchase in the RevenueCat dashboard.
...
**Expected Results:** Expected Results: The user loses access to the entitlement at the end of the current billing cycle and `EXPIRATION` event follows at the time of expiration.
...
**Preconditions:** App installed, subscripion groups set up properly in App Store Connect, and the current logged-in app user ID has a subscription.
...
- **iOS:** The new subscription is started on time based on apples timing for whether this was an upgrade, downgrade, or a crossgrade.
...
**Related Docs**
[Identifying Users](revenuecat.com/docs/customers/user-ids)
...
2. **Log in with a custom App User ID**
   ...
   - Use the `.logIn("<my_app_user_id>")` method to log in with a custom App User ID. This hsould also result in the new app user ID being aliased with the anonymous app user ID.
...
2. Close the app, create a new app user ID .
