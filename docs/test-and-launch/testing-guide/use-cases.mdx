...
<details>
  ...
  **Description:** Verify that a user's subscription/purchase flow works correctly including starting a subscription, having the subscription renew, and letting the subscription expire. If you are using non-subscription purchases, this will test if the purchase is able to go through. 
  ...
  **Expected Results:** Purchase goes through successfully and the user is able to see the purchase in the RevenueCat dashboard. 
...
<details>
  ...
  **Steps:**
  ...
  2. When a user tries to issue a refund, verify that you immediately direct them to the Apple support page: https://support.apple.com/en-us/HT204084
  3. Once a user gets a refund granted through Apple, verify that the refund has been seen by us by checking the customer history of this user 24 hours later and ensuring that a `CANCELLATION` event has occurred with the reason field set to `CUSTOMER_SUPPORT`.

  **Expected Results:** Users are able to request a refund and either you grant the refund via the Dashboard or API and you are able to successfully grant them that refund or point them in the correct direction. 
...
<details>
  ...
  **Pass/Fail Criteria:** Subscription is canceled and the event is sent. 

</details>
<details>
  <summary>Expiration</summary>

  **Description:** To verify that a user's subscription has expired, follow this test guide. 
  ...
  **Expected Results:** The User is able to cancel the subscription and the `EXPIRATION` event follows at the time of expiration. 

  **Pass/Fail Criteria:** Verify that the user no longer has access to the entitlement after the expiration has been triggered. 

</details>
<details>
  <summary>Upgrades and Downgrades</summary>
  ...
  **Preconditions:** App installed and the current logged-in app user ID has a subscription.
  ...
  **Pass/Fail Criteria:** Subscription is started on time based on the `PRORATION` method that you use. 
...
<details>
  ...
  **Steps:**
  ...
  2. Start a subscription purchase and ensure that the free trial or intro offer is being attached. 
...
### Configuration
<details>
  <summary>Anonymous App User ID Configuration</summary>
...
<RCCodeBlock tabs={[
  ...
  **Steps:**
  ...
  3. You should see [Purchases] - DEBUG: 👤 Initial App User ID - $RCAnonymousID:9be80d6558664c649fc0be8d00c5113a 
...
<details>
  <summary> Identified App User ID Configuration</summary>

  **Description:** Verify that the user has been successfully configured with the SDK with an identified ID. 
...
<RCCodeBlock tabs={[
  ...
  **Steps:**
  1. Open the App.
  2. Open your RevenueCat debug logs.
  3. You should see [Purchases] - DEBUG: 👤 Initial App User ID - [my_app_user_id]
...
<details>
  ...
  **Related Docs** 
  *  [RevenueCat Documentation - Google Offers](#) *(Replace with the actual link to the relevant doc)
...
2. **Handling Developer-Determined Offers:**
   - Offers with the `rc-ignore-offer` tag are not automatically applied, and developer-determined offers are handled according to the setup in the Google Play Console.
...
<details>
  ...
  **Related Docs** 
  * [RevenueCat Documentation - Displaying Products](#) *(Replace with the actual link to the relevant doc)*

  **Preconditions:** 
    * The RevenueCat SDK is integrated into the app.
    * Offerings are configured in the RevenueCat dashboard.
    * The app's paywalls are set up to display offerings and packages.
...
- **Fail:** If any scenario does not behave as expected—such as incorrect fetching, failure to display offerings, or incorrect handling of empty states—the test fails.
...
2. **Log in with a custom App User ID**
   - Use the `.logIn("<my_app_user_id>")` method to log in with a custom App User ID.
...
1. **Initial Log In with Custom App User ID:**
   - The `customerInfo` object should correctly reflect the custom App User ID (`<my_app_user_id>`), showing associated subscription status and purchase history.
...
4. **Switching Accounts:**
   - When switching from one custom App User ID to another, the `customerInfo` should update to reflect the new custom App User ID correctly, without merging unless explicitly required by the app’s merge logic.
...
- **Fail:** If any scenario does not behave as expected—such as incorrect merging, failure to update `customerInfo`, or retention of previous IDs—the test fails.
...
<details>
  ...
  **Preconditions:** App installed, Two subscriber IDs, first subscriber ID has a current subscription, another subscriber ID has no subscription. 

  **Steps:**
  1. Open the app with the currently subscribed app user ID.
  2. Close the app, and create a new [sandbox user](https://www.revenuecat.com/docs/test-and-launch/sandbox).
...
<details>
  ...
  **Preconditions:** App installed, Two subscriber IDs, first subscriber ID has a current subscription, other subscriber ID has no subscription. 
...
<details>
  ...
  **Preconditions:** App installed, Three app user ID's, first app user ID has a current subscription, second app user ID has no subscription, and the third app user ID has a subscription to the same product. 
...
<details>
  ...
  **Preconditions:** App installed, two app user ID's, the first app user ID has a subscription, the second one does not. 
  ...
  **Pass/Fail Criteria:** User is correctly blocked from making the purchase and causing a transfer. 
</details>
<details>
  ...
  **Expected Results:** Subscription and entitlements are transferred to the second app user ID. This app user ID also has a new alias with the original app user ID. 

  **Pass/Fail Criteria:** User is correctly aliased in the customer history and the new user has the entitlements of the old user. 
</details>
...
<details>
  ...
  **Steps:**
  ...
  6. Verify that the purchase went through by checking the customer history of the app user ID and verifying that the purchase event has been seen. 
...
<details>
  ...
  **Preconditions:** App installed, Paywall set up in Revenuecat, fresh app user ID, device and store account is set to the country that's localization is to be tested. 
...
**Related Docs:**
- RevenueCat Documentation - Subscription Lifecycle *(Replace with the actual link to the relevant doc)*
...
1. **Initialize the SDK and ensure it’s correctly configured.**
...
<details>
  ...
  **Description:**
Test the application and eligibility of free trials for subscriptions through RevenueCat Billing, ensuring that the correct trial period is applied based on the customer's eligibility criteria.

**Related Docs:**
- RevenueCat Documentation - Free Trials *(Replace with the actual link to the relevant doc)*
...
**Description:**
Test the refund process to ensure that subscriptions are correctly refunded, and the customer immediately loses access to entitlements associated with the subscription.

**Related Docs:**
- RevenueCat Documentation - Managing Customer Subscriptions *(Replace with the actual link to the relevant doc)*

**Preconditions:**
- A subscription is active and has been paid for by the customer.
- The RevenueCat SDK is integrated and configured.

**Example Code:**
```javascript
// Example: Refunding a subscription
// Use the REST API or dashboard to trigger a refund
curl -X POST https://api.revenuecat.com/v1/projects/proj1ab2c3d4/subscriptions/sub1a2b3c4d5e/actions/refund \
