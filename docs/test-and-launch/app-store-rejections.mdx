...
Your app is finally complete! 🎉 You're tracking subscriptions in Sandbox, your Testflight users love it, you've followed our [Launch Checklist](/docs/launch-checklist), and you're finally ready to launch your app to the world.
...
If an error occurs during the purchase process, it will be one of the errors outlined in our [Error Handling guide here](/docs/errors). Understanding what error triggered the rejection is the only way to make sure the issue gets resolved. 
...
This is actually the generic error message Apple uses for all [SKErrors](https://developer.apple.com/documentation/storekit/skerror) and not an error message from RevenueCat. See our [guide on Error Handling](/docs/errors) for more info.
...
Double check that you're [checking the subscription status correctly](/docs/purchaserinfo) and all of the [products are added to Entitlements](/docs/entitlements) as needed.
