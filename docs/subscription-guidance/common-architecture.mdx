...
![Entitlement on Backend](/images/entitlement_on_backend_screenshot.png)
...
Process Purchase: When a user selects an offering and makes a purchase, the SDK processes the transaction.

Complete Purchas: The SDK finalizes the purchase, and the results are sent to the RevenueCat backend for further processing.
...
Query Database for Subscription Status: The app queries the backend database to check the user’s subscription status.
