...
!!!! info here about minimum SDK version / the experimental flag !!!!

### Purchasing A Subscription with Add-Ons

When purchasing a mult-line subscription with RevenueCat, the package passed into the PurchaseParams’ constructor will be used as the base item of the multi-line subscription, and the packages passed in to the .addOnPackages() function will be treated as add-on items. You can purchase a multi-line subscription with the SDK like so:
...
- Purchasing multi-line subscriptions is currently unavailable in our Paywalls product.
...
- Any add-ons marked for deferred removal are expired
