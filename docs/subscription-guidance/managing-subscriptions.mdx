...
For information about Stripe, you can read more about working with web payments [here](/stripe#working-with-web-payments).
...
| **DEFERRED**                        | Replacement takes effect when the old plan expires, and the new price will be charged at the same time. Google recommends this mode for downgrading to a less expensive tier, and for changing recurring period on the same tier (from monthly to annual). **Important**: [Google Server Notifications](/google-server-notifications) are required to be configured for `DEFERRED` mode to work properly. |
...
#### [PRODUCT_CHANGE](/customer-history#event-types) events and webhooks
...
For Apple transactions,  prorated revenue **will not** be shown in the [Customer History](/customer-history) page. However, prorated revenue **will** be calculated for your chart and overview data.

### Google Play

For Google transactions, prorated revenue will be shown in the [Customer History](/customer-history) page and will be calculated for your chart and overview data.

## Cancelling Subscriptions

Cancelling (or unsubscribing) from subscriptions is handled differently on each platform. Cancellations are automatically detected by RevenueCat within a few hours of occurring. This detection time can be increased to near real-time by enabling [Platform Server Notifications](/server-notifications).
...
Deleting a user from RevenueCat **WILL NOT** cancel their subscription. The user can still trigger the [Restore Purchases](/making-purchases#restoring-purchases) method to re-sync their transactions with RevenueCat servers.
:::