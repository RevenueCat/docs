...
| ---------------------- | ------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| Introductory Offers    | New Users                 | Not required              | Applied to eligible purchases automatically. [How to check eligibility](/docs/subscription-guidance/subscription-offers#checking-eligibility).                                                                                                                                                                                                                              | ✅           |
...
| ⚠️ **Not recommended** | New and Existing Users    | Not Required              | Treated as a regular purchase, revenue will not be accurate in [Charts](/docs/dashboard-and-metrics/charts) and [Integrations](/docs/integrations/webhooks) due to Apple/StoreKit limitations. Codes don't auto-renew, aren't compatible with `presentCodeRedemptionSheet`, restricted to non-commercial use, and restricted to [1,000 codes every 6 months](https://help.apple.com/app-store-connect/#/dev50869de4a). | ❌           |

## In-App Purchase Keys

For RevenueCat to securely authenticate and validate a Subscription Offer request with Apple, you'll need to upload an In-App Purchase Key following our [guide](/docs/service-credentials/itunesconnect-app-specific-shared-secret/in-app-purchase-key-configuration).

:::tip Introductory Offers
An In-App Purchase Key is not required for Introductory Offers, only for Promotional Offers and Offer Codes. The Purchases SDK will automatically apply an introductory offer to a purchase. [Documentation](/docs/getting-started/entitlements/ios-products#adding-introductory-offers-and-free-trials) for reference.
:::
...
![Subscription Offers are created as new pricing options in App Store Connect](https://files.readme.io/fd483c8-Screen_Shot_2019-04-17_at_3.32.04_PM.png)
...
![Screen Shot 2020-12-01 at 10.02.15 AM.png](https://files.readme.io/4a0ab13-Screen_Shot_2020-12-01_at_10.02.15_AM.png)
...
When users click your link within your app to redeem the offer code, it will take them outside of the app to complete the purchase. It is important to call [syncPurchases](/docs/getting-started/restoring-purchases#syncpurchases) when the user returns back to your app to retrieve their purchase. This may be done by recording when the user leaves the app due to the link, and calling `syncPurchases` when the user returns to the app. If not, the user may need to [trigger a restore](/docs/getting-started/restoring-purchases) within your app when they come back.
