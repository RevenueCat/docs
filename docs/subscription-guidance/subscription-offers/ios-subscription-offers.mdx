...
|------------------------------------------------------------|--------------------------|---------------------------------------------------------------------------------------------------------|
| [Through the App Store with Streamlined Purchasing Enabled](https://www.revenuecat.com/docs/subscription-guidance/subscription-offers/ios-subscription-offers#redeeming-a-win-back-offer-through-the-app-store-with-streamlined-purchasing-enabled)  | Should not be used if you require a subscriber to perform an action before purchasing (e.g. signing in) |
| [Through Your App From a StoreKit Message](https://www.revenuecat.com/docs/subscription-guidance/subscription-offers/ios-subscription-offers#redeeming-a-win-back-offer-in-your-app-from-a-storekit-message)                   | Deferring when the message is displayed requires SDK version 5.6.0+                                                                    |
| Through the App Store with Streamlined Purchasing Disabled |                                                                                                                                        |
| Through Your Paywall                                       | Requires SDK version 5.10+                                                                                                                                        |
| Through Your App with Your Own IAP Code                    | More info on using your own IAP code [here](https://www.revenuecat.com/docs/migrating-to-revenuecat/sdk-or-not/finishing-transactions) |
| Through a StoreKit StoreView                               |                                                                                                                                        |
...
You can disable Streamlined Purchasing in App Store Connect under *Apps* → *Your App* → *Subscriptions* → *Streamlined Purchasing*.

![disable_streamlined_purchasing.png](/images/win-back-offers/disable_streamlined_purchasing.png)

After a user starts a win-back redemption flow on the App Store and opens your app, the RevenueCat SDK will call the `PurchasesDelegate.purchases(readyForPromotedProduct:startPurchase)` function to let your app know that the user has started a redemption flow. Your app should perform whatever actions it needs to do before the redemption flow continues. When your app is ready for the user to complete the redemption flow, you should call the `startPurchase` callback provided in the `readyForPromotedProduct` function.

import iosWinBackOfferRedemptionAppStoreStreamlinedPurchasingDisabled from "!!raw-loader!@site/code_blocks/subscription-guidance/subscription-offers/ios-winback-offer-redemption-app-store-streamlined-purchasing-disabled.swift";
<RCCodeBlock tabs={[
    {
        type: 'swift',
        title: "Swift",
        content: iosWinBackOfferRedemptionAppStoreStreamlinedPurchasingDisabled,
    }
]}/>
...
import ios-winback-offers-fetch-eligible-winback-offers from "!!raw-loader!@site/code_blocks/subscription-guidance/subscription-offers/ios-winback-offers-fetch-eligible-winback-offers.swift";
<RCCodeBlock tabs={[
    {
        type: 'swift',
        title: "Swift",
        content: ios-winback-offers-fetch-eligible-winback-offers,
    }
]}/>

Once the user has selected a win-back offer that they'd like to redeem, you can purchase it with the SDK:

import ios-winback-offers-purchase-winback-offer from "!!raw-loader!@site/code_blocks/subscription-guidance/subscription-offers/ios-winback-offers-purchase-winback-offer.swift";
<RCCodeBlock tabs={[
    {
        type: 'swift',
        title: "Swift",
        content: ios-winback-offers-purchase-winback-offer,
    }
]}/>

#### Redeeming a Win-Back Offer in a StoreKit StoreView

You can also redeem win-back offers in a StoreKit StoreView (paywall) with the following SwiftUI code:

import ios-winback-offer-storekit-storeview from "!!raw-loader!@site/code_blocks/subscription-guidance/subscription-offers/ios-winback-offer-storekit-storeview.swift";
<RCCodeBlock tabs={[
    {
        type: 'swift',
        title: "Swift",
        content: ios-winback-offer-storekit-storeview,
    }
]}/>
