...
You may directly access the cached virtual currencies using the `cachedVirtualCurrenices` property. This is helpful for rendering UI immediately, or for supporting showing virtual currencies when there is no network connection. Keep in mind that this value is cached and isn't guaranteed to be up to date.
