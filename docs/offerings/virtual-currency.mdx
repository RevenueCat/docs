...
Virtual currency functionality is available in the native iOS SDK in versions 5.X.X and above.
...
You may directly access the cached virtual currencies using the `cachedVirtualCurrencies` property. This is helpful for rendering UI immediately, or for supporting showing virtual currencies when there is no network connection. Keep in mind that this value is cached and isn't guaranteed to be up to date.
