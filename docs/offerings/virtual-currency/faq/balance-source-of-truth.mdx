...
If you've already implemented a virtual currency system on your backend (e.g: for web), it likely makes sense to continue treating your backend as the source of truth and rely on RevenueCat webhooks to update balances when purchases are made or when subscription lifecyle changes.
...
:::info
RevenueCat does not track the actual balance in this model. Instead, it notifies your backend of balances that were granted from purchases / subscription lifecycles.
:::
