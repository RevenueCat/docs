...
RevenueCat provides comprehensive event tracking for virtual currency transactions, allowing you to monitor and respond to balance changes in real-time through webhooks.

## Timeline Events

Virtual currency transactions appear in the [Customer History](/dashboard-and-metrics/customer-history) timeline, providing visibility into when currency was granted or removed from a customer's balance. These events help with debugging, customer support, and understanding customer behavior.
...
| ---------------------- | ------------------------------------------------------------------------------- | ------------------------------ |
| Granted ... [Currency] | Currency was added to the customer's balance through a purchase or subscription | `VIRTUAL_CURRENCY_TRANSACTION` |
| Removed ... [Currency] | Currency was removed from the customer's balance due to a refund                | `VIRTUAL_CURRENCY_TRANSACTION` |

### Event Details

When you click on a virtual currency event in the Customer History, you can view additional details including:

- The amount of currency granted or removed
- The currency type and description
...
| `adjustments[].currency`             | Object  | Details about the virtual currency type involved in the transaction.                                                      |
...
| `adjustments[].currency.description` | String  | A description of the virtual currency.                                                                                    |
| `product_display_name`               | String  | The display name of the product that triggered the virtual currency transaction.                                          |
| `purchase_environment`               | String  | The environment where the product purchase was made.                                                                      |
| `source`                             | String  | The source of the virtual currency transaction.                                                                           |
