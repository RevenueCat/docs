...
| Cancellation                | Custom              | rc_cancellation_event          | A subscription or non-renewing purchase has been cancelled. See [cancellation reasons](/event-types-and-fields#cancellation-and-expiration-reasons) for more details.                                                                       | ✅         | ✅          | ✅      | ✅      | ✅     |
...
| Expiration                  | Custom              | rc_expiration_event            | A subscription has expired and access should be removed. If you have [Platform Server Notifications](/server-notifications) configured, this event will occur as soon as we are notified (within seconds to minutes) of the expiration.    | ✅         | ✅          | ✅      | ✅      | ✅     |
...
| Product Change              | Custom              | rc_product_change_event        | A subscriber has changed the product of their subscription. This does not mean the new subscription is in effect immediately. See [Managing Subscriptions](/managing-subscriptions) for more details on updates, downgrades, and crossgrades. | ✅         | ✅          | ❌      | ✅      | ❌     |
...
In order to associate RevenueCat data with the Iterable User Profile, either the RevenueCat `$email` or `$iterableUserId` [Subscriber Attributes](/subscriber-attributes) should be set in RevenueCat. The preferred method is to send the `$email` attribute. If neither of these fields exist, RevenueCat will fallback to the RevenueCat app user ID. You can read more about Iterable user profiles in Iterable's [Identifying the User](https://support.iterable.com/hc/en-us/articles/360035402531-Identifying-the-User-) documentation.
...
While still on the Customer View, select the purchase event in the [Customer History](/customer-history) page and make sure that the Iterable integration event exists and was delivered successfully.
