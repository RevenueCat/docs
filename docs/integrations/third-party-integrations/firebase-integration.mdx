...
To check access to entitlements, you can either [use the RevenueCat SDK](/getting-started/getting-started#10-get-subscription-status) or use Firebase Authentication custom claims. For example, to check whether the current user has access to an entitlement called `premium`, you could use the following Firebase code:
