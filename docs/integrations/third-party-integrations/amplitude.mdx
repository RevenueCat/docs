...
### Attach next cycle price for trial starts
...
Please note that the estimate is based on the latest price we've observed for the purchased product in the country and currency of the trial start. Price changes, accepted promotional offers, upgrades/downgrades, and brand new products being used may all result in these estimates being absent or inaccurate.

### Attach next cycle price for renewal events (Optional)

If you want to attach the next cycle price for renewal events, toggle the Attach next cycle price for renewal events field to `Yes` in the Amplitude integration settings. The next cycle price will be attached as the `revenue` field in the event.

### Attach next cycle price for cancellation events (Optional)
