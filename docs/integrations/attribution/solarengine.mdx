...
2. Enter your **iOS App Key** and/or **Android App Key**. These keys authenticate requests from RevenueCat to Solar Engine. You can find them in your Solar Engine dashboard.
...
4. Provide event names for each lifecycle event RevenueCat will send, or choose **Use default event names** to populate the recommended mapping:
   ...
   | RevenueCat lifecycle event | Default Solar Engine event name | Required |
   | :------------------------- | :------------------------------ | :------- |
   | Initial purchase           | `initial_purchase_event`        | ✅       |
   | Trial started              | `trial_started_event`           | ✅       |
   | Trial converted            | `trial_converted_event`         | ✅       |
   | Trial cancelled            | `trial_cancelled_event`         | ✅       |
   | Renewal                    | `renewal_event`                 | ✅       |
   | Cancellation               | `cancellation_event`            | ✅       |
   | Non-renewing purchase      | `non_renewing_purchase_event`   | Optional |
   | Uncancellation             | `uncancellation_event`          | Optional |
   | Subscription paused        | `subscription_paused_event`     | Optional |
   | Expiration                 | `expiration_event`              | Optional |
   | Billing issue              | `billing_issue_event`           | Optional |
   | Product change             | `product_change_event`          | Optional |

   Customize any names to match the event definitions you have configured inside Solar Engine.
