...
If you attempt to serve this Paywall on a lower SDK version we'll provide a fallback Paywall. [Learn more](https://www.revenuecat.com/docs/paywalls/displaying-paywalls#default-paywall).
...
Some variables are not available on all version of the SDK. If an unavailable variable is found on an older version of the SDK, RevenueCatUI will automatically display a [default paywall](/paywalls/displaying-paywalls#default-paywall) to avoid showing a broken paywall.
