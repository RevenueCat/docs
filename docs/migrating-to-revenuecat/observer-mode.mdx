...
If using RevenueCat alongside Unity IAP or other plugin that includes the Android InAppBillingService class, follow [these instructions ](/installation/unity#installation-with-unity-iap-side-by-side)
:::
...
Certain integrations, such as most of the [Attribution Integrations](/attribution), require some device data to work properly. **Observer Mode does not automatically collect any device data**.
...
- With Observer Mode up-and-running capturing new purchases, you may want to [migrate existing purchases to RevenueCat as well ](/migrating-existing-subscriptions)
