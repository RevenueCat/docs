...
3. When integrating your server with Apple's server to server notifications, you can use either _Version 1_ or _Version 2_, but _Version 2_ notifications are recommended to use features such as [auto-detecting price changes](/subscription-guidance/price-changes).
