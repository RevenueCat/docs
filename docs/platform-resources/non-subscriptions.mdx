...
|  Stripe  | One-time purchases supported when using [Stripe Checkout](/docs/getting-started/stripe) |
...
For more information, read our guide on [Entitlements](/docs/getting-started/entitlements).
...
:::info
If you haven't already, you'll need to set up products in the respective app stores on which you intend to ship your app. For more information on setting up products and adding them to RevenueCat, check out the [Configuring Products](/docs/getting-started/entitlements#configuration) guide.
:::
...
For more information displaying products, check out the [Displaying Products](/docs/getting-started/displaying-products) guide.
...
:::warning
It's important to note that at the moment, logic for keeping track of consumable redemptions must be handled outside of RevenueCat. We recommend your server is set up to receive [Webhook](/docs/integrations/webhooks) events for `NON_RENEWING_PURCHASE` to appropriately provide consumable content for your users.
...
For more information on making purchases, check out the [Making Purchases](/docs/getting-started/making-purchases) guide.
