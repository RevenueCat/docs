...
|  Stripe  | One-time purchases supported when using [Stripe Checkout](/docs/stripe) |
...
For more information, read our guide on [Entitlements](/docs/entitlements).
...
:::info
If you haven't already, you'll need to set up products in the respective app stores on which you intend to ship your app. For more information on setting up products and adding them to RevenueCat, check out the [Configuring Products](/docs/entitlements#configuration) guide.
:::
...
For more information displaying products, check out the [Displaying Products](/docs/displaying-products) guide.
...
:::warning
It's important to note that at the moment, logic for keeping track of consumable redemptions must be handled outside of RevenueCat. We recommend your server is set up to receive [Webhook](/docs/webhooks) events for `NON_RENEWING_PURCHASE` to appropriately provide consumable content for your users.
...
For more information on making purchases, check out the [Making Purchases](/docs/making-purchases) guide.
