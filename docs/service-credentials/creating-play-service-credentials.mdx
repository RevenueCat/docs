...
In the Google Play Console, go to the '**Users and Permissions**' section' and select invite user. You'll want to invite the service account you created or the one provided by the automated setup script. Under 'App permissions' you need to add your app. Then under 'Account permissions', you need to grant certain permissions in order for RevenueCat to properly work.
