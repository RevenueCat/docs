...
When creating your service account for the first time, this would be enabled by default. However, due to new [policy changes](https://cloud.google.com/blog/products/identity-security/automatically-disabling-leaked-service-account-keys-what-you-need-to-know) introduced by Google, it is possible that your keys were automatically disabled. Please navigate to your [Google Cloud console](https://console.cloud.google.com/iam-admin/serviceaccounts) to confirm the status of your credentials. We do not recommend re-enabling your credentials due to security risks. Instead, we recommend generating new credentials following the instructions outlined in this documentation.
...
Below is the most commonly occurring errors when setting up your service credentials or developer notifications and what you should to do when you see them.
