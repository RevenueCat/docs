...
Note that while our [credentials guide](/creating-play-service-credentials) and our [developer notifications guide](/google-server-notifications) will move back and forth between the Google Play Console, the Google Cloud Console, and the RevenueCat dashboard, we've combined the steps into categories for ease of use.
