...
---
For RevenueCat to securely validate purchases through Apple StoreKit 2, authenticate and validate [Subscription Offers](/docs/subscription-guidance/subscription-offers/ios-subscription-offers) request with Apple and enable [customer lookup](/docs/dashboard-and-metrics/customer-lists#find-an-individual-customer) via Order ID for iOS apps, you need to upload an in-app purchase key, and you'll also need to provide an Issuer ID.
