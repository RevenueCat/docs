...
If you're looking to use Google's installment plans, all you'll need to do is create is base plan for an Installment subscription in Google Play Console! You can access the installment plan details from the SDK by using the `SubscriptionOption.installmentsInfo` property, like this:
```kotlin
