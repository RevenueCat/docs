...
RevenueCat's Customer Center is currently in beta, and only available on iOS.
:::
...
- Configuration is done via JSON in the RevenueCat dashboard
...
For this beta version, we've decided to focus on the SDK, and on getting early feedback on the functionality. The customer center section of the dashboard is mainly a JSON editor of the configuration that will be sent to the SDK. This is not meant to be final and it's built this way for the purpose of getting early feedback on the feature. We are going to be replacing it with a full UI where you will be able to configure what your users see, and how the different sections of the view behave.
...
The default JSON is configured for the following behavior:
...
1. **MANAGEMENT**: This is the main screen users see when they have an active subscription. It contains several paths (or actions) that users can take:
   ...
   - Change Plans
   - Cancel Subscription
   Each path has an `id`, a `title_key` (which references a localized string), and a `type` that defines the action.
...
Remember that the `title_key`, `subtitle_key`, and each path's `title_key` should correspond to keys in your localization configuration to ensure proper translation across different languages.
...
1. **Missing Purchase** (`MISSING_PURCHASE`):
   - Used for restoring previous purchases.

2. **Refund Request** (`REFUND_REQUEST`):
   - Allows users to request refunds.

3. **Change Plans** (`CHANGE_PLANS`):
   - Enables users to switch between different subscription plans.

4. **Cancel** (`CANCEL`):
   - Provides an option for users to cancel their subscription.
