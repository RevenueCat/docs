...
Paywalls v2 does not support passing in a custom font provider via the `PaywallFontProvider` method that original Paywalls used. Instead, in the near future you'll be able to configure Paywalls to use the font providers you've already installed in your app directly from the Dashboard. Using this method will have no effect on Paywalls v2.
