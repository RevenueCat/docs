...
<YouTubeEmbed videoId="k7_SEjGccgI" title="Targeting by Placement" />
...
If you want to use Placements to serve unique Offerings at different paywall locations for all customers, you can do so by creating a Targeting Rule that targets "Any audience" as its condition. Then, order that rule at the bottom of your list of Live rules if you want other audience-specific Targeting Rules to supercede it.
:::

Then, select the Offering you want to display, and enter the identifier of the Placement you want to serve that Offering on. If this is your first time using that Placement, you'll need to click **Add** to add its identifer to your list. Once you've saved this rule, that Placement will be available to select in other Targeting Rules.
...
import flutterContent5 from "!!raw-loader!@site/code_blocks/tools/targeting_by_placement_5.dart";
import capacitorContent6 from "!!raw-loader!@site/code_blocks/tools/targeting_by_placement_6.ts";
import unityContent7 from "!!raw-loader!@site/code_blocks/tools/targeting_by_placement_7.cs";
...
<RCCodeBlock
  tabs={[
    ...
    { type: "react-native", content: reactNativeContent4, name: "React Native" },
    { type: "capacitor", content: capacitorContent6, name: "Capacitor" },
    { type: "unity", content: unityContent7, name: "Unity" },
  ]}
/>
...
Yes you can! When creating an Experiment, you'll have the same options to add Offerings per Placement like you can when creating Targeting Rules to A/B test the impact of changes made at any Placement in your app.
