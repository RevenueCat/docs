...
If you publish a paywall that includes unsupported components, customers on older SDK versions may not be able to render it and will fall back to RevenueCat's default paywall.

To avoid that, you can create a **fallback paywall** and use **Targeting** to serve it only to customers on older SDK versions.
...
Give the duplicate a clear suffix like `-fallback` or `-legacy` (for example, `onboarding_v3_fallback`) so it’s obvious which paywall/Offering is intended for older SDK versions.
:::::
...
1. **Save to draft** while you’re iterating.
2. **Publish** the fallback paywall once it’s ready to serve.

:::::warning Keep the fallback paywall purchase-ready
Make sure the fallback paywall still includes everything required to convert (packages, a purchase button, and any required legal links), since this is the paywall older customers will actually see.
:::::
...
Targeting Rules are evaluated from top to bottom. Keep your “older SDK → fallback Offering” rule **above** any broader rules (like “Any audience”) so older customers don’t get matched by a more general rule first.
:::::
