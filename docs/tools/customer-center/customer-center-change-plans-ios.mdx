...
Under the hood, the RevenueCat SDK uses Apple's [SubscriptionStoreView](https://developer.apple.com/documentation/storekit/subscriptionstoreview) from StoreKit to present the product change interface. This ensures a native, Apple-approved experience for subscription management that follows iOS design guidelines and handles the subscription change process securely through the App Store.
...
In the Customer Center configuration, you can specify exactly which products from your subscription group should be presented as change options for each of your current subscription products.
