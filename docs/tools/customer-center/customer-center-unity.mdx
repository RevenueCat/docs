...
2. Add the OpenUPM scoped registry (if not already added). Go to your project's settings → Package Manager, and add a new scoped registry with URL `https://package.openupm.com` and scopes: `com.openupm` and `com.revenuecat.purchases-unity-ui`.
...
import eventsLocalUnity from '@site/code_blocks/tools/customer-center-events-unity-local.cs?raw';

<RCCodeBlock
  tabs={[
    { type: "unity", content: eventsLocalUnity, name: "Unity" },
  ]}
/>
