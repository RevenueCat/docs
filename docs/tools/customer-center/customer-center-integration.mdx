...
The Customer Center will automatically show promotional offers based on specific user actions during the cancellation or refund process. Here’s how it works:

Cancellation Offers: By default, for responses in the cancellation survey, the system will use a promotional offer labeled with the suffix `rc_cancel_offer`. RevenueCat will automatically look for and display this promotion if the offer id in App Store Connect ends with rc_cancel_offer.

Refund Offers: By default, when a user requests a refund, RevenueCat will show the promotional offer with the suffix `rc_refund_offer` in the promotion offer id.
...
You can customize both refund and cancellation ids in the Advanced Configuration JSON:
```
"promotional_offers": {
  "cancel_promo_offer_key": {
    "ios_offer_id": $YOUR_CUSTOM_CANCELLATION_ID,
  },
  "refund_promo_offer_key": {
    "ios_offer_id": $YOUR_CUSTOM_OFFER_ID,
  }
},
```
