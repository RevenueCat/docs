...
sidebar_label: Kotlin Multiplatform Integration
...
CustomerCenter for Kotlin Multiplatform is available on Maven Central and can be included via Gradle.
...
[versions] 
purchases-kmp = "`${sdkVersions.ios}`"

[libraries]
purchases-core = { module = "com.revenuecat.purchases:purchases-kmp-core", version.ref = "`${sdkVersions.ios}`" }
purchases-ui = { module = "com.revenuecat.purchases:purchases-kmp-ui", version.ref = "`${sdkVersions.ios}`" }
```
