...
[Learn more about setting unique values for customers eligible for intro offers.](/tools/paywalls/creating-paywalls-v2/customer-states)
...
Alternate text can be configured for each image to make them more accessible.

To configure atl. text, first open that image in the media gallery, which you can accesss by clicking on `Replace image` within an image component's properties. Then, hover of the image you want to edit and click the eye icon that appears to view its details.
...
[Learn more about selected state styling.](/tools/paywalls/creating-paywalls-v2/customer-states)
