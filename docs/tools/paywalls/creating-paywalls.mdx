...
Once configured, the exit offer will automatically be presented when users attempt to dismiss the paywall without purchasing. You must also use one of the supported presentation methods: `presentPaywall` in iOS and cross-platform SDKs, or `PaywallDialog` / `PaywallActivity` on Android.
