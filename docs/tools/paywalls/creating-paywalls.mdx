...
The templates above that support multiple packages allow you to select a package name which will be displayed when this package is being presented. Below is a small explanation of each option that you can choose from:
...
Some variables are not available on all version of the SDK. If an unavailable variable is found on an older version of the SDK, RevenueCatUI will automatically display a [default paywall](/docs/tools/paywalls/displaying-paywalls#default-paywall) to avoid showing a broken paywall.
To prevent this, you may use [Targetting](/docs/tools/targeting) to control who can see a paywall with a newer variable.
