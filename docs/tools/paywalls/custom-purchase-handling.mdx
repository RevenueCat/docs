...
Using RevenueCat Paywalls with your own purchase infrastructure follows a straightforward pattern:

1. Configure your products in the RevenueCat dashboard
2. Design your paywalls remotely using RevenueCat's Paywall Designer
3. Leverage the applicable RevenueCat SDK to fetch and render paywalls natively in your app
4. When users attempt a purchase, restore, etc. through the RevenueCat Paywall, your existing purchase infrastructure handles the transaction
...
[Configure your products](/projects/configuring-products) in the RevenueCat dashboard to match your existing product SKUs.
...
In the paywall integration above, you were provided with `performPurchase` and `performRestore` callbacks with placeholder implementations. These callbacks are where you integrate your existing purchase infrastructure.
...
Learn more about [setting up Targeting rules](/tools/targeting#creating-targeting-rules).
...
- Explore [Experiments](/tools/experiments-v1) to optimize conversion rates
