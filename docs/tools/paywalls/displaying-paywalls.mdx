...
:::warning
`PaywallFooterContainerView` is not compatible with React Native's new architecture in Android.
If you are using React Native 0.74, set `newArchEnabled=false` in `gradle.properties` to disable the new architecture to be able to use it.
:::
...
When using `PaywallView` or `PaywallFlutterView`, you may use one of the provided listeners to react to user actions.
Available listeners at this time are:
- onPurchaseStarted
- onPurchaseCompleted
- onPurchaseError
