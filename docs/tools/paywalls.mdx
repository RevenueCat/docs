...
Lastly, you'll need to change the pod specification from `PurchasesHybridCommon` to `PurchasesHybridCommonUI`. This needs to happen either in your iOS project's `Podfile`, or your Compose Multiplatform module's `build.gradle.kts`, depending on [how your Multiplatform module is integrated with your iOS project](/docs/getting-started/installation/kotlin-multiplatform#link-the-native-ios-sdk).
