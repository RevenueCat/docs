...
import {
  ...
  getOfferingForEUR,
} from "./web-billing-doc-snippets";
import type { Package, Product } from "@revenuecat/purchases-js";
import { Purchases } from "@revenuecat/purchases-js";

const WEB_BILLING_PUBLIC_API_KEY = "rcb_sb_NLgftxAMfaRjBxlpJjSfQnGAQ";

test('Snippet "Configuring SDK" works', () => {
  const purchases = configuringSDK(WEB_BILLING_PUBLIC_API_KEY);
  expect(purchases).toBeDefined();
  purchases.close();
});

test('Snippet "Configuring SDK Anonymous" works', () => {
  const purchases = configuringSDKWithAnonUser(WEB_BILLING_PUBLIC_API_KEY);
  expect(purchases).toBeDefined();
  purchases.close();
});
...
test('Snippet "Get current offering" works', async () => {
  ...
  const callback = vi
    .fn()
    .mockImplementation((availablePackages: Package[]) => {
...
test('Snippet "Get custom offering" works', async () => {
  ...
  expect(
    (pkgs ?? ([] as Package[])).find(
      (pkg) => pkg.identifier == "experiment_group_package"
    )
  ).toBeDefined();
  Purchases.getSharedInstance().close();
});
